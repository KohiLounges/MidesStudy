...
$num_eval = 0; // Inicializar la variable $num_eval
...
if(isset($_GET['revisar'])){
    ...
    $num_eval_result = $num_eval_query->fetch();

    if ($num_eval_result) {
        $num_eval = $num_eval_result['num_evaluaciones'];
    }
                        ...
                        <?php
                           if(existeNota($alumno['id'],$id_materia,$conn) > 0){
                                ...
                                foreach ($registrosnotas as $eval => $nota){
                                    ...
                                    echo '<td><input type="text" maxlength="5" value="'.$nota['nota'].'" name="evaluacion' . $eval . 'alumno' . $index . '" class="txtnota" oninput="validarNota(this)"></td>';
                                }
                                if($num_eval > $num_notas){
                                    ...
                                    for($i = $num_notas; $i < $dif + $num_notas; $i++) {
                                        ...
                                        echo '<td><input type="text" maxlength="5" value="'.$nota['nota'].'" name="evaluacion' . $i . 'alumno' . $index . '" class="txtnota" oninput="validarNota(this)"></td>';
                                    }
                                }
                            } else {
                                //extrayendo el numero de evaluaciones para esa materia seleccionada
                                for($i = 0; $i < $num_eval; $i++) {
                                    echo '<td><input type="text" maxlength="5" name="evaluacion' . $i . 'alumno' . $index . '" class="txtnota" oninput="validarNota(this)"></td>';
                                }
                            }
